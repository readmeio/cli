 ...
 * frontmatter beyond title/hidden/excerpt, and a title that slugifies to the
 * folder name (allowing a `-N` uniqueness suffix).
 *
 * The excerpt's *value* is deliberately not checked. The platform never
 * writes one, but this CLI stamps the tag's description as `excerpt` on the
 * pages it generates (see `buildTagIndexContent`), so its presence alone
 * can't mean "hand-edited". Nor can its value: this check only runs under
 * `apply-tag-changes`, where the spec owns every live tag page's excerpt
 * and rewrites it each sync, and by the time a folder is being cleaned up
 * its tag has typically left the spec — taking with it the only description
 * the excerpt could be compared against. Comparing against the current
 * spec's tags would misread every such page as hand-edited and flatten it
 * into a stale `<tag>.md` rather than deleting it.
 */
function isGeneratedTagIndex(indexPath) {
  let parsed;
  try {
 ...
 */
function cleanupTagFolder(dir, { refDir, takenSlugs, changes }) {
  let entries;
  try {
  ...
  if (!indexFile || isGeneratedTagIndex(path.join(dir, indexFile))) {
...
function syncOneOas(refDir, oasFilename, spec, takenSlugs) {
  ...
  for (const dir of vacatedDirs) {
    ...
    cleanupTagFolder(dir, { refDir, takenSlugs, changes });
  }
